...
def query_parse(GIVEN_QUERY):
    ...
    parts_all=[]
    region_ids_query=[]
    for i in regionDic_query:
        for key ,value in i.items():
            region_ids_query.append(key)

    pois_query_parts=[]
    poiNotGo_query_parts=[]
    region_ids_query_parts=[]



    for record in partRecords:
        parts[record['id']] = record
        parts_all.append(record['id'])
    ...
    poi_ids_satisfy_parts=list(set(pois_query_parts)-set(poiNotGo_query_parts))

    poi_ids_region_ids_satisfy_parts=list(set(poi_ids_satisfy_parts).intersection(set(region_ids_query_parts)))

    poi_ids_satisfy_parts=list(set(poi_ids_satisfy_parts)-set(poi_ids_region_ids_satisfy_parts))
    ...
    return parts_all, nextPartsOf, startParts, endParts, pois, parts, places, \
            schedulePois, schedulePlaces, poiTags, placePoisMapping, currencies, poiCalendar, \
            poi_ids_satisfy_parts, countryIds_query, days_query, regions_query, regionDic_query, \
            pois_query, regionNotGo_query, poiNotGo_query, regionSorted_query, availableMonths_query, price_query, \
            hotelRating_query, arrivalRegionId_query, departRegionId_query
