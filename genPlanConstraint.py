...
def getPathDetail(order, parts, pois, schedulePois, places, schedulePlaces, poiTags, placePoisMapping , currencies, poiCalendar):
    base_price = 0.0
    amount = 0.0
    days = 0
    self_drive = False
...
def deduPoiFromNeighborPartsConstraint(stack,parts,broadPois):
    poiList=[]
    poi_idsFormer=parts[stack[-2]]['poi_ids']
    poi_idsFormer_copy=copy.deepcopy(poi_idsFormer)
    poiList.extend(poi_idsFormer_copy)
    poi_idsLater=parts[stack[-1]]['poi_ids']
    poi_idsLater_copy=copy.deepcopy(poi_idsLater)
    poiList.extend(poi_idsLater_copy)

    if len(poiList)!=len(set(poiList)):
        res=[]
        poiList_count=Counter(poiList)
        for part ,count in poiList_count.items():
            if count>1:
                res.append(part)
        for poi in list(res):
            if broadPois[poi]["type"] in ["Pois::Meal", "Pois::Attraction", "Pois::Activity"]:
                return False
    return True



def getDays(stack,parts):
    days=0
    lastPart=None
    for part in stack:
        couldMerge = False
...
def deduPoiConstraint(node,poiList,broadParts,broadPois):
    poiInNode=broadParts[node]['poi_ids']
    depuPoi=[i for i in poiInNode if i in poiList ]

    if depuPoi:
        for poi in list(depuPoi):
            if broadPois[poi]["type"] in ["Pois::Meal", "Pois::Attraction", "Pois::Activity"]:
                return False
    return True
